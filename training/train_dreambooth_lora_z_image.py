...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Local folder containing the training images plus a metadata.jsonl (HF imagefolder format). "
            "Passed as data_dir to load_dataset() when --dataset_name=imagefolder, since load_dataset() "
            "given a bare local path otherwise mis-infers the builder from any .jsonl file present and "
...
def main(args):
    ...
    # NOTE: upstream raises here citing pytorch#99272, but that's fixed on this system's
    # PyTorch (2.14.0) — verified bf16 conv2d forward+backward works fine on this MPS
    # device, and fp32 here was measured at ~250s/image for latent caching (impractical),
    # while fp16 hangs indefinitely in GradScaler's inf-check on MPS. bf16 needs neither.
